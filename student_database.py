...
def addStu(): 
    ...
    query = "insert into stu values('" + rino + "','" + rno + "','" + nam + "','" + dp + "',"+   str(cgpa) + ")" 
    cur.execute(query) 
    cn.commit() 
...
def searchStu(): 
    ...
    if c==1: 
        ...
        query = "select * from stu where regno='" + rn + "' " 
        cur.execute(query) 
        results = cur.fetchall() 
        if cur.rowcount <= 0: 
        ...
        query = "select * from stu where dept='" + dp + "' " 
        cur.execute(query) 
        results = cur.fetchall() 
        if cur.rowcount <= 0: 
            print("SORRY! NO MATCHING DETAILS AVAILABLE.") 
        ...
        dp = input("Enter Department name to search :") 
        query = "select count(name) from stu group by dept having  dept='" + dp + "' " 
        cur.execute(query) 
        results = cur.fetchall() 
        c=0 
        for i in results: 
...
def editStu(): 
    ...
    if ans == "y" or ans == "Y": 
        ...
        query = "update stu set dept='" + dp + "',cgpa=" + str(cgpa) +   " where regno='" + rn + "' "
        cur.execute(query) 
        cn.commit() 
        print("\nRECORD UPDATED") 
        print("\n")
...
def delStu(): 
    ...
    if ans == "y" or ans == "Y": 
        query = "delete from stu where regno='" + rn + "' " 
        cur.execute(query) 
        cn.commit() 
        print("\n## RECORD DELETED  ##") 
        print("\n")
